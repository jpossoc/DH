...
total recaudado */
select servicio_extra.nombre as nombre_servicio ,forma_pago.nombre as forma_de_pago, count(reserva.id) as cantidad, round(sum(reserva.importe)) as total_recaudado from reserva
join forma_pago 
on forma_pago.id = reserva.forma_pago_id
join servicio_extra 
